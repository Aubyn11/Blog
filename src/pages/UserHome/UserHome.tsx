...
import { ArrowRight, BookOpen, Tag, Clock, UserX } from 'lucide-react'
...
const UserHome: React.FC = () => {
  ...
  useEffect(() => {
    ...
    homeConfigService.getHomeConfig(userId)
      .then(data => {
        if (data) {
          setConfig({ ...DEFAULT_CONFIG, ...data })
        }
        // data 为 null 时使用默认配置，不算 notFound（用户存在但未配置主页）
      })
  ...
  if (notFound) {
    return (
      <div className="py-20 flex flex-col items-center justify-center text-center px-4">
        ...
        <h2 className="text-2xl font-bold text-gray-700 mb-2">用户不存在</h2>
        <p className="text-gray-400 mb-6">该用户主页不存在或已被删除</p>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {config.primaryBtnText && (
                <Link
                  to={config.primaryBtnLink || blogLink}
